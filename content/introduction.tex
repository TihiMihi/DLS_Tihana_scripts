...
Most data reduction software in \ac{mx} use \ac{sh} as a basis for absorption correction calculations. This includes DIALS (Winter 2018), the reduction software standardly used at Diamond Light Source.
...
Characterising the shape and orientation in detail has previously been done using optical microscopy (Leal 2008, Strutz 2011). An alternative approach taken by I23 to obtain the 3D model is X-ray tomography, which has been applied to characterise and visualise crystals (Merrifield 2011, Warren 2013).
...
\begin{figure}
    ...
    \caption{Cartoon model of the absolute path lengths of X-rays through the crystal sample used as a basis for analytically calculating absorption coefficients of sample material. Adapted by Lu \textit{et al.} \cite{LuSubmitted}.
